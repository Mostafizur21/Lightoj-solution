...
int lca_query(int node,int cost)
{
    for(int i=logg; i>=0; i--)
    {
        if(parent[node][i]!=-1&&dist[parent[node][i]]>=cost)
            node = parent[node][i];
    }
    return node;
}
