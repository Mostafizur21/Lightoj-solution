...
int main()
{
    int test,n,t,i,left[1001],right[1001],left_to_right[1000],right_to_left[1000],dp_left[1001],dp_right[1001];
    sc1(test);
    rep(t,test)
    {
        sc1(n);
        ...
        For(i,2,n)
        {
            dp_left[i] = min((dp_left[i-1]+left[i]),(dp_right[i-1]+right_to_left[i-1]+left[i]));
            dp_right[i] = min((dp_right[i-1]+right[i]),(dp_left[i-1]+left_to_right[i-1]+right[i]));
        }
        printf("Case %d: %d\n",t,min(dp_left[n],dp_right[n]));
    }
    return 0;
}
